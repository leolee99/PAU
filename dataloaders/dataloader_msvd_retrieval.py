...
class MSVD_DataLoader(Dataset):
    ...
    def __init__(
        ...
        for video_id in video_ids:
            ...
            cat_cap_list = []
            for idx, cap in enumerate(captions[video_id]):
                cap_txt = " ".join(cap)
                if subset == 'train_':
                    cat_cap_list.append(cap_txt)
                    if (idx % 2 == 0 and idx != 0) or (idx == len(captions[video_id])) :
                        cat_cap = ". ".join(cat_cap_list)
                        self.sentences_dict[len(self.sentences_dict)] = (video_id, cat_cap)
                        cat_cap_list = []
                else:
