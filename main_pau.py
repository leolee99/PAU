...
def get_args(description='PAU on Retrieval Task'):
    ...
    parser.add_argument("--tau", default=5, type=int, help="the scale parameter of the exponential function")
...
def init_model(args, device, n_gpu, local_rank):
    ...
    model = PAU.from_pretrained(args.cross_model, cache_dir=cache_dir, state_dict=model_state_dict, task_config=args)
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        loss, loss_set = model(input_ids, segment_ids, input_mask, video, video_mask)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                logger.info("Epoch: %d/%s, Step: %d/%d, Lr: %s, Loss: %f, Sim_Loss: %f, Uct_Loss: %f, Div_loss: %f, Time/step: %f", epoch + 1,
                            args.epochs, step + 1,
                            len(train_dataloader), "-".join([str('%.9f'%itm) for itm in sorted(list(set(optimizer.get_lr())))]),
                            float(loss), float(loss_set['sim_loss']), float(loss_set['uct_loss']), float(loss_set['div_loss']),
...
def _run_on_single_gpu(model, batch_list_t, batch_list_v, batch_sequence_output_list, batch_seq_features_list, batch_visual_output_list):
    ...
    for idx1, b1 in enumerate(batch_list_t):
        ...
        for idx2, b2 in enumerate(batch_list_v):
            ...
            b1b2_logits, ret, *_tmp = model.get_similarity_logits(sequence_output, seq_features, visual_output, input_mask, video_mask,
                                                                     loose_type=model.loose_type)
            b1b2_logits = b1b2_logits.cpu().detach().numpy()
            vu_logits = ret['vu'].cpu().detach().numpy()
            tu_logits = ret['tu'].cpu().detach().numpy()
            each_row.append(b1b2_logits)
            vu_each_row.append(vu_logits)
            tu_each_row.append(tu_logits)
        each_row = np.concatenate(tuple(each_row), axis=-1)
        vu_each_row = np.concatenate(tuple(vu_each_row), axis=0)
        tu_each_row = np.mean(np.concatenate(tuple(tu_each_row), axis=-1), axis=1, keepdims=True)
        sim_matrix.append(each_row)
        vu_vector.append(vu_each_row)
        tu_vector.append(tu_each_row)
    vu_vector = np.mean(np.concatenate(tuple(vu_vector), axis=1), axis=1, keepdims=True)
    tu_vector = np.concatenate(tuple(tu_vector), axis=0)

    return sim_matrix, {'vu_vector': vu_vector, 'tu_vector': tu_vector}

def eval_epoch(args, model, test_dataloader, device, n_gpu, epoch=100):
...
def main():
    ...
    if args.do_train:
        ...
        for epoch in range(resumed_epoch, args.epochs):
            # if args.local_rank == 0:
            #     save_model(epoch, args, model, optimizer, 0, type_name="")
